...
/*   Updated: 2026/03/28 00:05:51 by mchanlia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "miniRT.h"

static bool	fill_line(t_scene *scene, char *input, size_t	*obj_idx)
{
	char	**split_l;
	...
	else
	{
		...
		if (!ft_strcmp("pl", split_l[0]))
			return (fill_plane(scene, split_l, *obj_idx));
...
t_scene	*fill_scene(char **av)
{
	...
	return (get_next_line(-42), close(fd), scene);
}
